...
def check_rows_cols():
    ...
    for i in range(N):
        ...
        max_count = max(max_count, get_max_countinue(arr[i]), get_max_countinue(cols))
    return max_count
    ...
    for j in range(N):
        for swap in swaps:
            ...
            if 0 <= dx < N and 0 <= dy < N:
                if (i, j, dx, dy) not in dp and (dx, dy, i, j) not in dp:
                    swap_value(i, j, dx, dy)
                    total_max = max(total_max, check_rows_cols())
